...
const CanvasBoard = forwardRef(({ mode, gridSize, setGridSize, tool, paintColor, setUsedColors, setAddedColors }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    generatePattern, undo, download, saveProject, loadProject, clear
  }));
  ...
  const clear = () => {
    if (!offCanvasRef.current) return;
    saveState(); // 지우기 전 상태를 저장해서 실행 취소(Undo)가 가능하게 만듭니다.
    const ctx = offCanvasRef.current.getContext('2d');
    ctx.fillStyle = '#ffffff';
    ctx.fillRect(0, 0, offCanvasRef.current.width, offCanvasRef.current.height);
  ...
  const generatePattern = ({ type, img, targetColor }) => {
    ...
    // 투명 배경 이미지(PNG)를 대비해 무조건 흰색 배경을 먼저 채워줍니다.
  ...
  const processColors = (ctx, w, h, type, targetColor) => {
    ...
    if (type === 'text' && targetColor) {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        // 텍스트는 밝기(Luminance)를 기준으로 판별하여 한글이 덜 뭉개지게 처리
        const brightness = r * 0.299 + g * 0.587 + b * 0.114;
        if (brightness < 160) { // 약간 밝은 픽셀까지 글자로 인식 (획 살리기)
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        if (a < 128 || (r > 240 && g > 240 && b > 240)) {
          r = 255; g = 255; b = 255; // 투명하거나 아주 밝은 테두리 픽셀은 흰색으로 날리기
        } else if (r < 30 && g < 30 && b < 30) {
          r = 0; g = 0; b = 0; // 아주 어두운 픽셀은 완벽한 검은색으로
        } else {
          const step = 85; // 기존보다 색상 단계를 크게 넓혀 잡다한 회색/기타 색을 대폭 줄임
